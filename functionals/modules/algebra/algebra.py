...
std    = lambda eqn: eqn if not iseqn(eqn) else f'{Algebra(eqn).lhs-Algebra(eqn).rhs}=0'    #AttributeError
...
def varz(expr: str) -> tuple:
    '''docstring'''
    return tuple(set(map(var, terms(expr))))
...
def common(expr: str) -> str:
    ...
    if len(a := alphaz(expr)) > 1:      # if there are more than one var
        return ''
    
    com_alpha = a[0]
    return com_num*(Algebra(com_alpha)**com_pow)
...
class Algebra:
    ...
    @classmethod
    def poly(cls, eqn: str) -> Algebra:
        '''Initializes the eqn as a ploynimial (lhs = 0)
        >>>Algebra.poly('x+y-4 = 0') -> Algebra('x+y-4')
        >>>Algebra.poly('2x+3y-7 = 4x-2y+6') -> Algebra('-2x+5y-13')'''
        if not iseqn(eqn): return cls(std(eqn))
        return cls(str(std(eqn)).replace(f'{sign(eqn)}0',''))
...
# etcetra:
    ...
    def roots(self) -> list:
        ...
        if eqns:
            ...
            coeff = lambda v, expr: [const(i) for i in expr if var(i) == v][0]
            val   = lambda v, eqn: -[i for i in terms(eqn) if type(i) == int][0]/coeff(v,eqn)

            def multivar_roots(*eqns) -> list:
                '''Returns the roots of the supplied eqns in pulti varaiable'''
                comV = lambda eq1, eq2: l[0] if (l:= list(set(varz(eq1)) & set(varz(eq2)))) else ''
                roots = []
                for eq1 in eqns:
                    for eq2 in eqns[1:]:
                        if (v:= comV(eq1, eq2)):    # if there is a common var
                            eq3, eq4 = eq1*coeff(v, eq2), eq2*coeff(v, eq1)
                            root_eq = std(eq3-eq4)
                            if len(vlist:= varz(root_eq)) == 1:
                                roots.append(val(vlist[0], root_eq))
                            else: roots.append(root_eq)

                if all(type(i) != Algebra for i in roots):
                    return roots
                else: return multivar_roots(*roots)
        
            # if linear equation in 2 var
            if len(eqns)==1 and len(varz(self))==2 and deg(self)==1:
